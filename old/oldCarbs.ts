...
export const oldCarbs = (meal, carbAbsTime) => {
	...
	const meals = meal.map(entry => ({ ...entry, time: moment(entry.time).valueOf() }));
	const timeSinceMealMin = meals.map(entry => ({ ...entry, mills: entry.time, time: (Date.now() - moment(entry.time).valueOf()) / (1000 * 60) }));
	logger.debug('this is the trimmed down meals and time since last meal:%o', timeSinceMealMin);
	...
	let timeSinceMealAct = karbs.map(entry => {
		...
		logger.debug('carbs_g:%o', carbs_g, 'fast:%o', fast, 'rest:%o', rest, 'fast_carbs:%o', fast_carbs, 'slow_carbs:%o', slow_carbs);
