...
const UVAsimulator_1 = require("../src/UVAsimulator");
...
describe('uva test default PATIENT', () => {
    test('basal  0.75 from TREATMENTS should generate flat sgv', () => {
        ...
        const yList = [];
        for (let i = 0; i < 12; i++) {
            const { x, y } = UVAsimulator_1.default({ env: { WEIGHT: '80' }, lastState, treatments, profiles: [], entries });
            yList.push(y.Gp);
            lastState = x;
        }
    ...
    test('basal 0.75 from PROFILE should generate flat sgv', () => {
        ...
        for (let i = 0; i < 12; i++) {
            const { x, y } = UVAsimulator_1.default({ env: { WEIGHT: '80' }, lastState, treatments: [], profiles: profile, entries });
            yList.push(y.Gp);
            lastState = x;
        }
        expect(lastState).toMatchSnapshot();
...
//# sourceMappingURL=uva.test.js.map