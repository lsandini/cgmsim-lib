...
function default_1({ treatments, profiles, pumpBasals }, env) {
    ...
    for (let currentAction = startDiaAction; currentAction.diff(endDiaAction) <= 0; currentAction.add(5, 'minutes')) {
        ...
        if (storedBasal) {
            basalAsBoluses.push(Object.assign({}, storedBasal));
        }
        else {
    ...
    utils_1.default.info('\x1b[32m', '------------------pumpBasals', pumpBasals.length, '\x1b[0m');
    const td = dia * 60;
    ...
    utils_1.default.info('this is the aggregated insulin activity from pump basal in the last dia hours:', resultPumpAct);
    const pumpBasalAct = JSON.stringify(resultPumpAct, null, 4);
    utils_1.default.info('the pump\'s basal activity is:', pumpBasalAct);
