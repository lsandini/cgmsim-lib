...
function default_1(weight, toujeos) {
    ...
    utils_1.default.debug(toujeo_data);
    ...
    let timeSinceToujeoAct = toujeo_data.map(entry => {
        ...
        return Object.assign(Object.assign({}, entry), { time: time, toujeoActivity: (dose * (S / Math.pow(tau, 2)) * time * (1 - time / td) * Math.exp(-time / tau)) / 60 });
    });
    utils_1.default.debug('the is the accumulated toujeo activity:%o', timeSinceToujeoAct);
    // compute the aggregated activity of last toujeos in 27 hours
    let lastToujeos = timeSinceToujeoAct.filter(function (e) {
        return e.time <= 30;
    });
    utils_1.default.debug('these are the last toujeos and activities:%o', lastToujeos);
    var resultTouAct = lastToujeos.reduce(function (tot, arr) {
        return tot + arr.toujeoActivity;
    }, 0);
    utils_1.default.debug('result: %0', resultTouAct);
    return resultTouAct;
}
exports.default = default_1;
//# sourceMappingURL=oldToujeo.js.map