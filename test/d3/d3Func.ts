...
export const line = ({
  ...
  lineColors: _lineColors = ['deepskyblue', 'gold', 'purple', 'darkgreen', 'red', 'orange', 'grey', 'blue', 'pink'],
  ...
  const _scale = (dim: 'x' | 'y') => {
    ...
    if (dim === 'y') {
      ...
      if (data.type === 'multiple') {
        const firstRo: ValueDataSource[] = data.values.length > 0 ? data.values[0] : [{ key: 0, value: 0, name: '' }];
