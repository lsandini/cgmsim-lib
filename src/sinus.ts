...
export default function (now: number) {
	...
	getLogger().debug('timestamp in milliseconds %o', now);

	// timestamp in days;
	getLogger().debug('timestamp in days %o', now / 86400000);

	// timestamp in days rounded;
	getLogger().debug('timestamp in days rounded %o', Math.floor(now / 86400000));
	...
	getLogger().debug('time of the day in 2 pi cycle %o', daycycle.toFixed(2));

	// time of the day in 360 deg cycle;
	const dayCycleDeg = (hours * 360) / 24;
	getLogger().debug(
		'time of the day in 360 deg cycle %o',
		dayCycleDeg.toFixed(2),
	);
	...
	getLogger().debug('sin cosin curves result: %o', sinCurves);
