...
export default function (timezone: string) {
	...
	logger.debug('[sinus] time of the day in hours - using decimals, not minutes: %o', time.toFixed(2));

	// express minutes also;
	// const hoursAbs = Math.floor(hours);
	// const minutes = (hours - hoursAbs) * 60;

	logger.debug('[sinus] express hours: %o', hours.toFixed(), 'minutes');
	logger.debug('[sinus] express minutes in minutes also: %o', minutes.toFixed(), 'minutes');

	// time of the day in 2 pi cycle;
	const dayCycle = (time * Math.PI) / 12;
	logger.debug('[sinus] time of the day in 2 pi cycle %o', dayCycle.toFixed(2));
	...
	logger.debug('[sinus] value of the sin function according to hours, oscillating from -1 to +1: %o', SIN.toFixed(2));

	// value of the sin function oscillating between 0 and 2;
	const sinInterm = Math.sin((dayCycleDeg * Math.PI) / 180) + 1;
	logger.debug('[sinus] value of the sin function oscillating between 0 and 2: %o', sinInterm.toFixed(2));
	...
	logger.debug('[sinus] value of the sin function oscillating around 1, +/- 20 %, starting from 1 and ending in 1: %o', sinCorr.toFixed(2));
	logger.debug('[sinus] When the time of day is ' + time.toFixed() + ' hours and ' + minutes.toFixed() + ' minutes, the sinusoidal value is: ' + sinCorr.toFixed(3));
	...
	logger.debug('[sinus] value of the cos function oscillating around 1, +/- 0.5, starting from 1.5 and ending in 1.5: %o', COScorr.toFixed(2));
	logger.debug('[sinus] When the time of day is ' + time.toFixed() + ' hours and ' + minutes.toFixed() + ' minutes, the cosinusoidal value is: ' + COScorr.toFixed(3));
