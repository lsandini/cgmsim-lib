...
function getProfileSwitch(treatments, duration) {
	const computedProfilesSwitch: { start: moment.Moment; end: moment.Moment; basal: { time: string, value: number }[] | number }[] = [];
...
function getBasalFromProfiles(orderedProfiles: Profile[], currentAction: moment.Moment) {
...
function activeBasalByTime(activeProfileBasals: { value: number; time: string; timeAsSecond?: number; }[]|number, currentAction: moment.Moment) {
...
export default function (treatments: Treatment[], profiles: Profile[], dia: number, peak: number) {
	...
	for (let currentAction = startDiaAction; currentAction.diff(endDiaAction) <= 0; currentAction.add(5, 'minutes')) {
		...
		const profilesStichActives = computedProfileSwitch
			.filter(t => (
				t.start.diff(currentAction) <= 0 &&
				t.end.diff(currentAction) > 0
			));
		...
		if (tempBasalActives.length > 0) {
			...
			const basal= activeBasalByTime(profilesStichActives[0].basal,currentAction);
			basalToUpdate = {
				minutesAgo: getDeltaMinutes(currentAction.toISOString()),
				insulin: basal / 12,
			};
			...
			let currentBasal = { value: getBasalFromProfiles(orderedProfiles, currentAction) };
			basalToUpdate = {
				minutesAgo: getDeltaMinutes(currentAction.toISOString()),
