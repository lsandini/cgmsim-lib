...
export default function carbs(
	...
	const meals = treatments
		...
		.filter((e) => e?.carbs > 0 && getDeltaMinutes(e.created_at) <= carbsAbs)
		.map((e) => ({
			...e,
	...
	const timeSinceMealAct = carbs.map((entry) => {
		...
		const fast = Math.min(Math.random() * entry.carbs, 40);
		...
		if (minutesAgo < fast_carbAbsTime / 2) {
			...
			fast_carbrate =
				((fast_carbs * 4) / fast_carbAbsTime) *
				(1 - minutesAgo / fast_carbAbsTime);
		...
		if (minutesAgo < slow_carbAbsTime / 2) {
			...
			slow_carbrate =
				((slow_carbs * 4) / slow_carbAbsTime) *
				(1 - minutesAgo / slow_carbAbsTime);
