...
export default class DailyProfile {
	...
	_interp1(tod: number): number {
		...
		if (tod < this.nodes[0][0]) return NaN;
	...
	_interp_smooth(tod: number): number {
		...
		return (
			this.nodes[p][1] + (this.nodes[k][1] - this.nodes[p][1]) * Math.pow(t_, 3) * (10 - 15 * t_ + 6 * Math.pow(t_, 2))
		);
	}
	...
	insertNode(newNode: ProfileNode): void {
		...
		const index = this.nodes.findIndex((node) => Math.abs(node[0] - newNode[0]) <= this.stepDuration);
		if (index === -1) {
			// if not, add new node
	...
	removeNodeAt(tod: number) {
		this.nodes = this.nodes.filter((node) => node[0] === 0 || Math.abs(node[0] - tod) > this.stepDuration);
	}
