...
import { Matrix, Vector } from '../types/CommonTypes';
...
export function frobeniusNorm(A: Matrix): number {
	return A.reduce((norm, ai) => norm + ai.reduce((rowsum, aii) => rowsum + aii * aii, 0), 0);
}
...
export function scalarMultiplyDiagonal(A: Matrix, scalar: number): Matrix {
	...
	return A.map((row, i) => row.map((val, j) => (i === j ? val * scalar : val)));
}
...
export function vectorToDiagonalMatrix(v: Vector): Matrix {
	...
	return Array.from({ length: size }, (_, i) => Array.from({ length: size }, (_, j) => (i === j ? v[i] : 0)));
}
