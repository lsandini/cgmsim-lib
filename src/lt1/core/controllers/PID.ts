...
import Controller, { ControllerOutput, Measurement, TracedMeasurement } from '../../types/Controller';
...
export default class PID extends AbstractController<typeof PIDParameters> implements Controller {
	/** time of last update */
	...
	update(t: Date, s: TracedMeasurement) {
		...
		if (isMultipleOfSamplingTime(t, params.samplingTime)) {
			...
			let iir = params.basalRate - (params.kP / 100) * e - (params.kI / 100) * this._eIntegral;
