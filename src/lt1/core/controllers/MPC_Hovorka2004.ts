...
import { Vector } from '../../types/CommonTypes';
...
import Controller, { AnnouncementList, ControllerOutput, Measurement, TracedMeasurement } from '../../types/Controller';
import { ModuleProfile } from '../../types/ModuleProfile';
import { ParameterDescriptions } from '../../types/ParametricModule';
import { PatientInput, PatientInputOverTime } from '../../types/Signals';
import AbstractController from '../AbstractController';
import Cambridge, { stateDescription, State, i18n_tooltip as stateTranslation } from '../models/Cambridge';
...
export default class MPC_Hovorka2004 extends AbstractController<typeof MPCParameters> implements Controller {
	/** time of last update */
	...
	update(t: Date, s: TracedMeasurement, a: AnnouncementList = {}) {
		...
		if (isMultipleOfSamplingTime(t, params.samplingTime)) {
			...
			const targetprojector = new TargetProjector(G, params.predictionHorizon, params.predictionStep, params.targetBG);
			...
			const u_opt = new LevenbergMarquardt((u) => {
				...
				return R;
			}).findMinimum(u0, lambda, reltol, (msg: string) => log.push(msg), params.lowerBound, params.upperBound);
			...
			const u = (t: Date) => {
				return { iir: iir, carbs: momentaryCarbIntake(a, t) };
			};
...
export class TargetProjector {
	 ...
	 */
	private computeTargetTrajectory(G: number, predictionHorizon: number, dt: number, targetBG: number): number[] {
...
export const MPCParameters = {
	...
	processNoise: {
		unit: '1',
		default: Object.fromEntries(Object.keys(stateDescription).map((e) => [e, { default: 1, step: 0.1 }])),
	},
	/** Measurement noise */
...
export const i18n_tooltip = {
	en: {
		...
		predictionHorizon:
			'This is the number of time steps the controller looks ahead. The total predicted time range is the product of prediction horizon and prediction timestep.',
		predictionStep:
			'The duration of one prediction step. The total predicted time range is the product of prediction horizon and prediction timestep.',
		kaggr:
			'The higher this value is chosen, the stronger the controller varies basal rate to achieve the desired glucose curve. Smaller values lead to softer course of action.',
		lowerBound: 'This is the lower limit of the control variable..',
		upperBound: 'This is the upper limit of the control variable.',
		processNoise: {
			_: 'The larger these value is chosen, the more the measurement is trusted. Large values indicate high uncertainty in the respective state of the system.',
			...stateTranslation.en,
		},
		measurementNoise:
			'The larger this value is chosen, the more the process model is trusted. High values indicate high uncertainties in the measurement model.',
	},
	...
	de: {
		...
		predictionHorizon:
			'So viele Zeitschritte blickt der Regler in die Zukunft. Der gesamte prädizierte Zeitraum ist das Produkt von Prädiktions-Horizont und Prädiktions-Schrittweite.',
		predictionStep:
			'Die Dauer eines Prädiktions-Zeitschritts. Der gesamte prädizierte Zeitraum ist das Produkt von Prädiktions-Horizont und Prädiktions-Schrittweite.',
		kaggr:
			'Je größer dieser Wert gewählt wird, desto stärker variiert der Regler die Basalrate, um den gewünschten Glukoseverlauf möglichst genau zu erreichen. Für kleinere Werte ist sein Vorgehen behutsamer.',
		lowerBound: 'Das ist die untere Beschränkung der Stellgröße.',
		upperBound: 'Das ist die obere Beschränkung der Stellgröße.',
		processNoise: {
			_: 'Je größer dieser Wert gewählt werden, desto mehr wird auf die Messung vertraut. Große Werte bedeuten hohe Unsicherheit im jeweiligen Zustand des Systems.',
			...stateTranslation.de,
		},
		measurementNoise:
			'Je größer dieser Wert gewählt wird, desto mehr wird auf die Prozessmodell vertraut. Hohe Werte bedeuten hohe Unsicherheiten im Messmodell.',
	},
};
