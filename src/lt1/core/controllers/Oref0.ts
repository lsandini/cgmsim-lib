...
export type Oref0Profile = Partial<
	TypedParameterValues<typeof Oref0Parameters, {}> & {
		type: 'current';
...
export default class Oref0 extends AbstractController<typeof Oref0Parameters> implements Controller {
	/** patient profile */
	...
	update(t: Date, s: TracedMeasurement, announcements: AnnouncementList = {}, m: ControllerOutput = {}) {
		...
		const meal = this.mealManager
			.update(announcements, (uid) => announcements[uid].start <= t)
		...
		const tempEnd = this.currenttemp?.deliverAt?.valueOf?.() || 0 + (this.currenttemp?.duration || 0) * 60e3;
