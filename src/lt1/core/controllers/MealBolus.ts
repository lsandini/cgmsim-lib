 ...
 */
export default class MealBolus extends AbstractController<typeof MealBolusParameters> implements Controller {
	...
	update(t: Date, _s: TracedMeasurement, announcements: AnnouncementList = {}) {
		...
		const upcomingIDs = this.manager.update(
			announcements,
			(uid) => announcements[uid].start <= new Date(t.valueOf() + parameters.premealTime * 60e3),
		);
		...
		for (const id of upcomingIDs) {
			...
			reason.push(
				'-> announced meal: ' + announcements[id].carbs + ' g at ' + announcements[id].start.toLocaleString(),
			);
			reason.push('-> carb factor: ' + parameters.carbFactor + ' U/(10g)');
			reason.push(
				'-> meal bolus: ' +
					announcements[id].carbs +
					' g * (' +
					parameters.carbFactor +
					' U/(10g) * 10) = ' +
					ibolus +
					' U',
			);
...
export const i18n_tooltip = {
	...
	de: {
		...
		carbFactor:
			'Der KE-Faktor beschreibt, wie viel Insulin benötigt wird, um eine Kohlenhydrateinheit (10g) auszugleichen.',
