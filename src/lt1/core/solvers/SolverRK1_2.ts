...
const norm = (x: NamedVector) => Math.sqrt(Object.values(x).reduce((acc, curr) => acc + Math.pow(curr, 2), 0));
		...
		while (t < tFinal.valueOf()) {
			while (true) {
				...
				if (step.e > this._tol) {
					...
					this._timeStep = Math.max(
						1,
						Math.min(60, Math.floor(this._timeStep * Math.min(2, 0.9 * Math.sqrt(this._tol / 2 / step.e)))),
					);
					// complete this step
					break;
				}
