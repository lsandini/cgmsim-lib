...
export default class Simulator {
	...
	setMeals(meals: Meal[]): void {
		...
		this.meals = meals.map((meal) => {
			...
			const duration =
				typeof meal.duration !== 'undefined' && meal.duration >= minMealDuration ? meal.duration : minMealDuration;
	...
	setExerciseUnits(exercises: Exercise[]): void {
		...
		this.exercises = exercises.map((exercise) => {
			...
			const duration =
				typeof exercise.duration !== 'undefined' && exercise.duration >= minExerciseDuration
					? exercise.duration
					: minExerciseDuration;
	...
	runSimulation(): SimulationResult[] {
		...
		const tmax: Date = this.options.tmax ?? new Date(t.valueOf() + 1 * MS_PER_HOUR);
	...
	protected _computeNextTime(t: Date): Date {
		...
		tNext = blocks.reduce(
			(acc, next) => Math.min(acc, next.getNextUpdateTime(t)?.valueOf() || Infinity),
	...
	protected _nextCarbIntakeChange(t: Date): number {
		return this.meals.reduce((tNext, meal) => {
			...
			const tend = new Date(start.valueOf() + duration! * MS_PER_MIN);
			if (start > t && start.valueOf() < tNext) tNext = start.valueOf();
			if (tend > t && tend.valueOf() < tNext) tNext = tend.valueOf();
			return tNext;
		}, Infinity);
	}
	...
	protected _newMealStartingAt(t: Date): number {
		return this.meals
			.filter((meal) => meal.start.valueOf() === t.valueOf())
	...
	protected _getTimeOfNextMealAnnouncements(t: Date): number {
		const tMealAnnouncements = this.meals
			.filter(
				(meal) =>
