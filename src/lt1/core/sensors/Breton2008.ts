...
import { NormalRandomNumberGenerator } from '../../types/RandomNumberGenerator';
...
export default class CGM_Breton2008 extends AbstractSensor<typeof Breton2008Parameters> implements Sensor {
	...
	protected _generateNoise(PACF: number): void {
		...
		if (this.straightLinesBuffer.length == 16) {
			...
			for (let k = 0; k < 15; k++) {
				const nextNoise = this.straightLinesBuffer.slice(k, k + 15).reduce((acc, curr) => acc + curr, 0) / 15;
