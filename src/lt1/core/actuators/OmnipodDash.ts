 ...
 */
export default class OmnipodDash extends AbstractActuator<typeof OmnipodDashParameters> implements Actuator {
	/** time of last update */
	...
	update(t: Date, c: ControllerOutput) {
		...
		if (dt > 0) {
			...
			totalIIR = (this._periodicOscillation((iir * dt) / MIN_PER_HOUR + ibolus, t) * MIN_PER_HOUR) / dt;
		} else totalIIR = iir;
