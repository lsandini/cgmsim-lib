...
import Sensor, { Measurement, PatientOutput } from '../types/Sensor';
import { TracedMeasurement } from '../types/Signals';
 ...
 * @implements {Sensor}
 */
export default abstract class AbstractSensor<Parameters extends ParameterDescriptions>
	extends AbstractParametricModule<Parameters, typeof CommonSensorParametersDescription>
	implements Sensor
{
	/** previous output */
	...
	getOutput(): TracedMeasurement {
		return Object.fromEntries(this.getOutputList().map((id: string) => [id, () => this.output[<keyof Measurement>id]]));
	}
