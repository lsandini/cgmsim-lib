...
import { NamedVector } from '../types/CommonTypes';
import { ModuleProfile } from '../types/ModuleProfile';
import ODEPatientModel from '../types/ODEPatientModel';
import { ParameterDescriptions, TypedParameterValues } from '../types/ParametricModule';
import Patient, {
	PatientInput,
	PatientOutput,
	PatientProfile,
...
import { PatientInputOverTime } from '../types/Signals';
import Solver, { Derivatives } from '../types/Solver';
 ...
 */
export default abstract class AbstractODEPatient<S extends StateDescription, P extends ParameterDescriptions>
	extends AbstractParametricModule<P, typeof CommonPatientParametersDescription>
	implements ODEPatientModel<TypedPatientState<S>>
{
	...
	abstract computeDerivatives(t: Date, x: TypedPatientState<S>, u: PatientInput): TypedPatientState<S>;
...
type ODEPatientClass<State extends PatientState, P extends ParameterDescriptions> = {
	new (
		parameters?: Partial<TypedParameterValues<P, typeof CommonPatientParametersDescription>>,
		...args: any[]
	): ODEPatientModel<State>;
};
 ...
 */
export function createPatientFromODE<S extends StateDescription, P extends ParameterDescriptions>(
	...
	return class extends odeModel implements Patient {
		...
		_patient: ODEPatientMixin<TypedPatientState<S>, P> = new ODEPatientMixin(this);
 ...
 * @mixin
 */
export class ODEPatientMixin<State extends PatientState, Parameters extends ParameterDescriptions> {
	/** current state */
	private x = {} as State;
	...
	update(t: Date, u: PatientInputOverTime) {
		const derivatives: Derivatives = (t_: Date, x_: NamedVector) => {
			const d = this._odeModel.computeDerivatives(t_, <State>{ ...x_ }, u(t_));
			return d;
		};
		...
		if (typeof this._odeModel.updateDiscontinuousStates !== 'undefined') {
			this.x = <State>this._odeModel.updateDiscontinuousStates(t, { ...this.x }, u(t));
		}
	}
	...
	getInitialState(): State {
		return Object.keys(this.x0 || {}).length > 0 ? { ...this.x0 } : { ...this.xeq };
	}
