...
import Controller, { ControllerInternals } from '../types/Controller';
import { ModuleProfile, ModuleTranslationList } from '../types/ModuleProfile';
import { ParameterDescriptions, ParameterValues } from '../types/ParametricModule';
import { PatientProfile } from '../types/Patient';
import { AnnouncementList, ControllerOutput, Measurement, TracedMeasurement } from '../types/Signals';
...
export default class ControllerUnion implements Controller {
	...
	getI18n() {
		// merge labels
		const i18n_label = this._controllers.reduce((acc, c) => {
		...
		// merge tooltips
		const i18n_tooltip = this._controllers.reduce((acc, c) => {
	...
	getInputList(): Array<keyof Measurement> {
		return [
			...new Set(
				this._controllers.reduce((acc, c) => (acc = [...acc, ...c.getInputList()]), [] as Array<keyof Measurement>),
			),
		];
	}
	...
	getParameterList(): Array<string> {
		return this._controllers.reduce((acc, c) => (acc = [...acc, ...c.getParameterList()]), [] as Array<string>);
	}

	getParameterDescription(): ParameterDescriptions {
		return this._controllers.reduce((acc, c) => (acc = { ...acc, ...c.getParameterDescription() }), {});
	}
	...
	update(t: Date, s: TracedMeasurement, a: AnnouncementList = {}, c: ControllerOutput = {}): ControllerOutput {
		this._internals = {};
		...
		for (const controller of this._controllers) {
			...
			this._internals.predictedBG = this._internals.predictedBG ?? controller.getInternals()?.predictedBG;

			this._internals.debug = [...(this._internals.debug || []), ...(controller.getInternals()?.debug || [])];

			this._internals.reason = [...(this._internals.reason || []), ...(controller.getInternals()?.reason || [])];
		}
	...
	getNextUpdateTime(t: Date) {
		...
		const tNext = this._controllers.reduce(
			(acc, c) => Math.min(acc, c.getNextUpdateTime(t)?.valueOf() || Infinity),
			Infinity,
		);
		if (isFinite(tNext)) return new Date(tNext);
		return undefined;
	}
	...
	setParameterValues(parameters: ParameterValues) {
		for (const controller of this._controllers) {
			...
			const params = Object.keys(parameters)
				.filter((id) => controllerParamList.includes(id))
