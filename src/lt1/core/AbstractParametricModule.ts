...
> implements ParametricModule
{
	 ...
	 */
	getParameterUnit(id: keyof TypedParameterValues<Parameters, CommonParameters>): string | undefined {
		...
		for (const paramName in newValues) {
			if (paramName in this._parameters) {
				...
				if ((<any>newValues[paramName])?.type === 'DailyProfile') {
					...
					typeof newValues[paramName] === typeof this._parameters[id] ||
 ...
 */
function getDefaultParameterValuesRecursive(config: ParameterDescriptions): ParameterValues {
		...
		Object.keys(config)
			.map(function (p) {
				...
				if (
					...
					return [p, getDefaultParameterValuesRecursive(<ParameterDescriptions>config[p].default)];
				} else if (defaultValue instanceof DailyProfile) {
					// clone default object
					return [p, DailyProfile.fromJSON(JSON.parse(JSON.stringify(defaultValue)))];
...
export const ParametricModuleBuilder = (getParameterDescription: Function) => {
	...
	class myModule extends AbstractParametricModule<typeof myParameterDescription, {}> implements ParametricModule {
