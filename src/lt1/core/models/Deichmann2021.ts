	...
	implements ODEPatientModel<State>
{
	...
	computeIIR(targetBG: number, t: Date): number {
		...
		return ((params.Ib * (params.kd + params.ka)) / params.ka) * (params.Vi * params.BW) * params.ke * 60e-6; // uU/min -> U/h
	}
	...
	computeDerivatives(t: Date, x: State, u: PatientInput): State {
		...
		// insulin kinetics
		const IIRb = ((params.Ib * (params.kd + params.ka)) / params.ka) * (params.Vi * params.BW) * params.ke; // uU/min -> U/h
...
export default createPatientFromODE<typeof stateDescription, typeof parameterDescription>(Deichmann2021);
