...
import { PatientInput, PatientOutput, StateDescription, TypedPatientState } from '../../types/Patient';
	...
	implements ODEPatientModel<State>
{
	...
	computeIIR(targetBG: number, t: Date): number {
		...
		const b =
			-F01eq * params.SID -
			params.k12 * Q1eq * params.SIT +
			params.k12 * params.SIT * Q1eq +
			EGP0 * params.SID -
			EGP0 * params.k12 * params.SIE;
	...
	computeDerivatives(t: Date, x: State, u: PatientInput): State {
		...
		return {
			// glucose subsystem
			Q1: -F01c - x.x1 * x.Q1 + params.k12 * x.Q2 - FR + UG + params.EGP0 * params.BW * (1 - x.x3),
...
export default createPatientFromODE<typeof stateDescription, typeof parameterDescription>(Cambridge);
