...
import { TypedPatientState, PatientInput, PatientOutput, StateDescription } from '../../types/Patient';
...
export class UvaPadova_T1DMS
	extends AbstractODEPatient<typeof stateDescription, typeof parameterDescription>
	implements ODEPatientModel<State>
{
	...
	Uid(t: Date, x: State, u: PatientInput): number {
		...
		return ((params.Vm0 + params.Vmx * x.X * (1 + params.rgamma * risk)) * x.Gt) / (params.Km0 + x.Gt);
	}
	...
	computeDerivatives(t: Date, x: State, u: PatientInput): State {
		...
		const EGP = Math.max(0, params.kp1 - params.kp2 * x.Gp - params.kp3 * x.XL + params.kxi * x.XH);
		...
		let kempt =
			params.kmin +
			((params.kmax - params.kmin) / 2) *
				(Math.tanh(alpha * (Qsto - params.bmeal * D_)) - Math.tanh(beta * (Qsto - params.cmeal * D_)) + 2);
		...
		dx_dt.SRHs = -params.rho * (x.SRHs - Math.max(sigma2 * (Gb - G) + params.Hb * params.nH, 0)); // (##)
...
export const parameterDescription = {
	...
	m1: { unit: '1/min', default: 0.19 }, // [Dalla Man, IEEE TBME, 2007]
	...
	kmin: { unit: '1/min', default: 0.008 }, // [Dalla Man, IEEE TBME, 2007]
	...
	f: { unit: '1', default: 0.9 }, // [Dalla Man, IEEE TBME, 2007]
...
export default createPatientFromODE<typeof stateDescription, typeof parameterDescription>(UvaPadova_T1DMS);
