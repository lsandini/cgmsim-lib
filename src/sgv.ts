...
//const logger = pino();
const sgv_start = (entries: Sgv[], { basalActivity, liverActivity, carbsActivity, bolusActivity }: CGMSimParams, isf: number) => {
	...
	const sgv_pump = Math.floor(oldSgv + (BGI_ins * 18) + (liverDeltaMinutesActivity * 18) + (carbsDeltaMinutesActivity * 18) );
